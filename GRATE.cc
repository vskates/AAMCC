...
int main()
{
    ...
    // The user will be asked for the nuclear name to simulate its collisions.
    ...
    for(G4int count=0;count<histoManager.GetIterations() ;count++) {
        ...
        TObjArray *nucleons;
        if(histoManager.GetReaderID())  *ain = (*reader)();
       // *ain = histoManager.GetReaderID() ? (*reader)() : reinterpret_cast<GlauberCollisionReader*>(reader.get())->GetNucleons(nucleons);
        ...
        //*ain = histoManager.GetReaderID() ? (*reader)() : reinterpret_cast<GlauberCollisionReader*>(reader.get())->GetNucleons(nucleons);
        if(!histoManager.GetReaderID()) *ain = reinterpret_cast<GlauberCollisionReader*>(reader.get())->GetNucleons(nucleons);
